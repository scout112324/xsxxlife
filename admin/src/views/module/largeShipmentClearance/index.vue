<template>
  <div class="container">
    ...
    <el-table v-loading="loading" :data="bigList">
      ...
      <el-table-column label="上门时间" align="center" prop="goTime"></el-table-column>
...
<script>
import {bigList, deleteBig, updateBig} from '@/api/module/largeShipmentClearance'

export default {
  name: 'largeShipmentClearance',
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    submitForm() {
      if (this.noticeId) {
        ...
        updateBig(params).then(response => {
    ...
    getList() {
      this.loading = true
      bigList(this.queryParams).then(response => {
          if (response.code === 200) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$modal.confirm('是否确认删除这条大件清运').then(function () {
        return deleteBig({id: row.id})
      }).then(() => {
        this.getList()
